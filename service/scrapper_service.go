...
func Scrape(url string, chatId string) (obj string, err error) {
	instance := os.Getenv("NITTER_INSTANCE")
	responseBody := lib.Request(url)
	...
	parsedWebpage.Find("div.timeline-item").Each(func(i int, t *goquery.Selection) {
		...
		tweet_text = strings.ReplaceAll(tweet_text, "\n", "")
		...
		if quoteInfo := t.Find("div.quote"); quoteInfo.Length() > 0 {
			...
			quote_text = strings.ReplaceAll(quote_text, "\n", "")
		}
	...
	dateNow := time.Now()
	filename := fmt.Sprint(chatId, "-", dateNow.Unix(), ".csv")

	s3FilePath, err := arutek.UploadS3Public(
		lib.S3_CLIENT,
		buf,
		fmt.Sprint("twitter-data-export/", filename),
		os.Getenv("S3BUCKET_NAME"),
		"application/csv",
	)
	if err != nil {
		arutek.LoggerWarn(err.Error())
		return
	}
	obj = os.Getenv("S3BUCKET_HOST") + s3FilePath
	return
}
